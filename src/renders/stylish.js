...
const getStylishFormat = (value, depth = 1) => {
  switch (value.type) {
    ...
    case 'deleted':
    case 'unchanged':
      return `${makeIndent(depth)}${symbols[value.type]} ${value.key}: ${
        value.value
      }`;
    case 'changed':
      return `${makeIndent(depth)}${symbols.deleted} ${value.key}: ${
        value.valueBefore
      }\n${makeIndent(depth)}${symbols.added} ${value.key}: ${
        value.valueAfter
      }`;
    default:
